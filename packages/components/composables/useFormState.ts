...
import { useComponentContext } from './useResolvers'
...
export const useFormState = (
  ...
  const resolver = useComponentContext()
  ...
  const result = {
    ...
    mode: _displayMode,
    useValue,
    isEditMode: computed(() => _displayMode.value === 'edit'),
  }
  ...
  function setProps<T>(f: Field<T>, d?: ControlDefinition) {
    d ??= resolver.findDefinitionById(f.control)
    if (!d) return
